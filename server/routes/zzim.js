...
//(2) post /pz - 찜 추가 (성공 시 1 반환, 에러 혹은 중복 값 존재 시 0 반환) /request 보낼때 body에 userid , cockid 값 입력 필요
router.post("/pz", controller.pluszzim);
// (3) post /dz - 찜 제거(성공 시 1 반환, 에러 혹은 쿼리 실행 불가능 시 0 반환) /request 보낼때 body에 userid , cockid 값 입력 필요
router.post("/dz", controller.deletezzim);
// (4) post /sz - 사용자가 누른 칵테일 아이디 반환 /request 보낼때 body에 userid
router.post("/sz", controller.showzzim);
